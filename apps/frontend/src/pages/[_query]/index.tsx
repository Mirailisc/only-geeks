...
export default function SearchQuery() {
  ...
  const BlogContentResults = ({ result }: { result: { search: Search } }) => {
    ...
    return (
      <>
        <h2 className="my-2 text-lg font-bold">Blogs</h2>
  ...
  const ProfileContentResults = ({ result }: { result: { search: Search } }) => {
    ...
    return (
      <>
        <h2 className="my-2 text-lg font-bold">Profiles</h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
    ...
    <>
      ...
      <div className="container mx-auto mt-4">
        <div className="text-xl font-semibold">
          Found {(data?.search.blogs?.length ?? 0) + (data?.search.users?.length ?? 0)} results for &ldquo;{query}
          &rdquo;
        </div>
        <Tabs defaultValue={type}>
          <TabsList className="w-full">
            ...
            <TabsTrigger
              className={cn('w-full', data?.search.blogs?.length === 0 && 'hidden')}
